...
<p align="center">
  <a href="../README.md">README Inglese</a> ·
  <a href="./USE_CASES.md">Use cases</a> ·
  <a href="./DEV_NOTES.md">DEV_NOTES</a>
</p>
...
Il deploy Docker usa una singola immagine applicativa `loom`: il frontend Angular viene compilato dentro il jar Spring Boot ed esposto direttamente dal backend. `compose.yml` avvia questa immagine insieme a PostgreSQL e Caddy.
...
Per compilare l'immagine unica dal codice presente nella cartella:
...
Il compose avvia tre servizi:

- `database`: PostgreSQL;
- `loom`: backend Spring Boot che serve sia API sia frontend Angular statico;
- `caddy`: reverse proxy HTTPS davanti a Loom. Terminazione TLS automatica via Let's Encrypt (se `TLS_ENABLED=true`), altrimenti solo HTTP su :80.
...
> Loom e PostgreSQL **non** sono pubblicati sull'host: vivono solo nella rete Docker.
> Caddy raggiunge Loom via `http://loom:8080`; Loom raggiunge Postgres via `jdbc:postgresql://database:5432/...`.
...
Il workflow `.github/workflows/publish-images.yml` viene eseguito a ogni push e anche manualmente da GitHub Actions. Costruisce e pubblica l'immagine unica su GHCR:

```text
ghcr.io/davide-leva/loom:<tag>
```
...
LOOM_IMAGE=ghcr.io/your-org/loom:latest
...
Per provare invece lo stesso packaging del deploy, usa la modalità Compose: compila l'immagine unica `loom:dev` con frontend incluso nel backend e avvia `database`, `loom`, `caddy` da `compose.dev.yml`.
...
./scripts/dev.sh --compose # build + up dello stack Docker di sviluppo
```
...
Per lo stack Docker di sviluppo:

```bash
./scripts/dev.sh --compose --detach
./scripts/dev.sh --compose --stop
```

Apri <http://localhost/>. In questa modalità non c'è hot reload: ogni modifica al frontend o al backend richiede una nuova build dell'immagine.
...
e restituisce un access token Bearer valido per 8 ore di default (`JWT_TTL_MINUTES=480`).
...
Usa HTTPS e configura gli eventuali proxy davanti all'applicazione per non registrare la query string di `/login`: il token è presente nella prima richiesta HTTP.
