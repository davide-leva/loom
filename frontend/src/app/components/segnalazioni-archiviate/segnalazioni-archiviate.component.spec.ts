...
import { SegnalazioniArchiviateComponent } from './segnalazioni-archiviate.component';
...
class SegnalazioniStub {
  segnalazioniArchiviate$ = new Subject<SegnalazioneSummary[]>();
  segnalazioniArchiviate = jest.fn(() => this.segnalazioniArchiviate$.asObservable());
...
describe('SegnalazioniArchiviateComponent', () => {
  let fixture: ComponentFixture<SegnalazioniArchiviateComponent>;
  let component: SegnalazioniArchiviateComponent;
  let router: Router;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [SegnalazioniArchiviateComponent],
      providers: [
        provideRouter([]),
    ...
    .overrideComponent(SegnalazioniArchiviateComponent, {
      set: { providers: [
    ...
    fixture = TestBed.createComponent(SegnalazioniArchiviateComponent);
  ...
  describe('segnalazioniFiltrate()', () => {
    beforeEach(() => {
      component.segnalazioni.set([
        ...
        segnalazione({ id: 3, title: 'slow dashboard', status: 'COMPLETED', issueType: 'IMPROVEMENT' }),
        segnalazione({ id: 4, title: 'misc', deletedAt: '2026-09-19T08:00:00.000Z' })
      ]);
    });

    it('excludes deleted issues', () => {
      expect(component.segnalazioniFiltrate().map(i => i.id)).toEqual([1, 2, 3]);
    });
    ...
    it('filters by status', () => {
      component.statusFilter = 'COMPLETED';
      expect(component.segnalazioniFiltrate().map(i => i.id)).toEqual([3]);
    });
  ...
  describe('load()', () => {
    it('stores issues on success', () => {
      ...
      fixture.detectChanges();
      segnalazioniApi.segnalazioniArchiviate$.next([segnalazione({ id: 1 }), segnalazione({ id: 2 })]);
      segnalazioniApi.segnalazioniArchiviate$.complete();
    ...
    it('sets error on failure', () => {
      currentProjectIdSignal.set(5);
      fixture.detectChanges();
      segnalazioniApi.segnalazioniArchiviate$.error(new Error('boom'));
      expect(component.error()).toBe('Non riesco a caricare le segnalazioni archiviate.');
    });

    it('does nothing when no project', () => {
      expect(segnalazioniApi.segnalazioniArchiviate).not.toHaveBeenCalled();
    });
  });
