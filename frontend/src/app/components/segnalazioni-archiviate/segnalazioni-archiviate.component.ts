...
import type { StatusSegnalazione, SegnalazioneSummary, ProjectUserSummary } from '../../services/segnalazioni/segnalazioni.service';
...
import { filtraSegnalazioni } from '../../services/segnalazioni/segnalazione-filtri';
...
@Component({
  selector: 'app-segnalazioni-archiviate',
      ...
      <div class="page-title">
        <div>
          ...
          @if (projects.currentProject(); as project) {
            <p>Segnalazioni archiviate del progetto <strong>{{ project.name }}</strong></p>
          } @else {
            <p>Seleziona un progetto dall'header per vedere le segnalazioni archiviate.</p>
          }
        </div>
        ...
        <p-table [value]="segnalazioniFiltrate()" [(selection)]="selected" selectionMode="multiple" dataKey="id"
          ...
          <ng-template pTemplate="header">
            <tr>
              ...
              <th pSortableColumn="archivedAt">Data archiviazione <p-sortIcon field="archivedAt" /></th>
            </tr>
          </ng-template>
          <ng-template pTemplate="body" let-segnalazione>
            <tr [class.segnalazione-interna]="segnalazione.internal">
              @if (canDelete()) {
                <td><p-tableCheckbox [value]="segnalazione" /></td>
              }
              ...
              <td>{{ segnalazione.archivedAt | date:'dd/MM/yyyy HH:mm' }}</td>
            </tr>
          </ng-template>
          <ng-template pTemplate="emptymessage">
            <tr><td [attr.colspan]="canDelete() ? 8 : 7">Nessuna segnalazione archiviata.</td></tr>
          </ng-template>
        </p-table>
      }
...
export class SegnalazioniArchiviateComponent {
  ...
  segnalazioniFiltrate(): SegnalazioneSummary[] {
    // excludeArchived:false — we want to see archived issues here.
  ...
  load(): void {
    ...
    this.error.set(null);
    this.segnalazioniApi.segnalazioniArchiviate(projectId).subscribe({
      next: (segnalazioni: SegnalazioneSummary[]) => {
      ...
      error: () => {
        if (sequence !== this.loadSequence || projectId !== this.projects.currentProjectId()) return;
        this.error.set('Non riesco a caricare le segnalazioni archiviate.');
        this.loading.set(false);
      }
    });
  }
  ...
  confirmPermanentDelete(): void {
    if (!this.canDelete() || this.selected.length === 0) return;
    this.confirmation.confirm({
      header: 'Eliminazione definitiva',
      message: `Stai per eliminare definitivamente ${this.selected.length} segnalazion${this.selected.length === 1 ? 'e' : 'i'} archiviat${this.selected.length === 1 ? 'a' : 'e'}. L'operazione non è reversibile.`,
      acceptLabel: 'Elimina',
      rejectLabel: 'Annulla',
  ...
  private permanentlyDelete(): void {
    const ids = this.selected.map(segnalazione => segnalazione.id);
    if (ids.length === 0) return;
    this.deleting.set(true);
    this.segnalazioniApi.eliminaDefinitivamenteSegnalazioni(ids).subscribe({
      next: () => {
  ...
  typeLabel(type: SegnalazioneSummary['issueType']): string {
