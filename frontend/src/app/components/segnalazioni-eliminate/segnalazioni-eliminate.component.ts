...
@Component({
  selector: 'app-segnalazioni-eliminate',
        ...
        <p-table [value]="segnalazioniFiltrate()" [(selection)]="selected" selectionMode="multiple" dataKey="id"
          ...
          <ng-template pTemplate="body" let-segnalazione>
            <tr [class.segnalazione-interna]="segnalazione.internal">
              ...
              <td>{{ segnalazione.deletedAt | date:'dd/MM/yyyy HH:mm' }}</td>
            </tr>
          </ng-template>
...
export class SegnalazioniEliminateComponent {
  ...
  load(): void {
    ...
    this.error.set(null);
    this.segnalazioniApi.segnalazioniEliminate(projectId).subscribe({
