...
import type { IssueField, IssueFieldOption, IssueSummary } from '../../shared/models/issue.types';

class IssuesStub {
  issueFields$ = new Subject<IssueField[]>();
  issueFields = jest.fn(() => this.issueFields$.asObservable().pipe());
  issueFieldOptions$ = new Subject<IssueFieldOption[]>();
  issueFieldOptions = jest.fn(() => this.issueFieldOptions$.asObservable().pipe());
  createIssue$ = new Subject<IssueSummary>();
  createIssue = jest.fn(() => this.createIssue$.asObservable());
  uploadAttachment$ = new Subject<any>();
  uploadAttachment = jest.fn(() => this.uploadAttachment$.asObservable());

  emitFields(fields: IssueField[]): void {
    this.issueFields$.next(fields);
    this.issueFields$.complete();
  }

  emitOptions(options: IssueFieldOption[]): void {
    this.issueFieldOptions$.next(options);
    this.issueFieldOptions$.complete();
  }

  emitFieldsError(err: any): void {
    this.issueFields$.error(err);
    this.issueFields$.complete();
  }
...
describe('IssueCreateDialogComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [IssueCreateDialogComponent],
      providers: [
        provideAnimations(),
  ...
  describe('save()', () => {
    ...
    it('does nothing when not canSave', () => {
      component.formModel.title = '';
      component.save();
      expect(issuesApi.createIssue).not.toHaveBeenCalled();
    });
    ...
    it('creates issue with trimmed title/description', () => {
      component.save();
      expect(issuesApi.createIssue).toHaveBeenCalled();
      const sent = issuesApi.createIssue.mock.calls[0][0];
    ...
    it('sets error and clears saving on failure', () => {
      component.save();
      issuesApi.createIssue$.error(new Error('boom'));
    ...
    it('emits created and uploads no attachments when none attached', () => {
      ...
      component.save();
      issuesApi.createIssue$.next({ id: 99 } as IssueSummary);
      expect(createdSpy).toHaveBeenCalledWith({ id: 99 });
      expect(issuesApi.uploadAttachment).not.toHaveBeenCalled();
      expect(component.saving).toBe(false);
    });
    ...
    it('uploads attachments and emits created on success', () => {
      ...
      component.save();
      issuesApi.createIssue$.next({ id: 99 } as IssueSummary);
      expect(issuesApi.uploadAttachment).toHaveBeenCalledWith(99, 7, file);

      issuesApi.emitUpload({ id: 1 });
      expect(createdSpy).toHaveBeenCalledWith({ id: 99 });
      expect(component.saving).toBe(false);
    });
    ...
    it('shows attachment error but still emits created on partial upload failure', () => {
      ...
      component.save();
      issuesApi.createIssue$.next({ id: 99 } as IssueSummary);
      issuesApi.uploadAttachment$.error(new Error('upload fail'));
  ...
  describe('schema loading via ngOnChanges', () => {
    it('loads fields and options when projectId is set', () => {
      ...
      expect(issuesApi.issueFields).toHaveBeenCalledWith(1);
      expect(issuesApi.issueFieldOptions).toHaveBeenCalledWith(1);
    });
    ...
    it('populates fields and groups options', () => {
      ...
      issuesApi.emitFields(fields);
      issuesApi.emitOptions(options);
    ...
    it('handles schema load error', () => {
      component.projectId = 5;
      component.ngOnChanges();
      issuesApi.emitFieldsError(new Error('boom'));
