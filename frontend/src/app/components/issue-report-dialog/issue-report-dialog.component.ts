...
import type { IssueStatus, IssueType, ProjectUserSummary } from '../../services/issues/issues.service';

interface Option<T> { label: string; value: T; }
type AssignmentFilter = number | 'ALL' | 'NONE';
type TypeFilter = IssueType | 'ALL' | 'NONE';
...
export class IssueReportDialogComponent {
  ...
  readonly statusOptions: Option<IssueStatus | 'ALL'>[] = [
  ...
  generate(): void {
    ...
    this.error = '';
    this.issuesApi.downloadIssueReport(projectId, {
