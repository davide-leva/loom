...
import { SegnalazioneFormComponent, SegnalazioneFormModel } from './segnalazione-form.component';
import type { SegnalazioneCampo, SegnalazioneCampoOpzione } from '../../shared/models/segnalazione.types';
...
describe('SegnalazioneFormComponent', () => {
  let fixture: ComponentFixture<SegnalazioneFormComponent>;
  let component: SegnalazioneFormComponent;

  function makeModel(): SegnalazioneFormModel {
    return { title: '', description: '', values: {}, attachments: {}, internal: false };
  }

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [SegnalazioneFormComponent]
    }).compileComponents();

    fixture = TestBed.createComponent(SegnalazioneFormComponent);
  ...
  describe('activeOptionsFor()', () => {
    it('returns only options flagged active=true', () => {
      const opt = (id: number, active: boolean) => ({ id, definitionId: 1, projectId: 1, value: `v${id}`, label: `V${id}`, active }) as SegnalazioneCampoOpzione;
