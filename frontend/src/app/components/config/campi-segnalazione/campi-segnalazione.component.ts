...
import type {
  ...
  Project
} from '../../../services/config/admin-config/admin-config.service';
...
type FieldDraft = SegnalazioneCampoInput;
type OptionDraft = Required<Pick<SegnalazioneCampoOpzioneInput, 'value' | 'label'>> & { active: boolean };

@Component({
  selector: 'app-campi-segnalazione',
  imports: [FormsModule, ButtonModule, CardModule, DialogModule, InputTextModule, Textarea, SelectModule,
    TableModule, EntityTableComponent, SegnalazioneFormComponent],
  templateUrl: './campi-segnalazione.component.html',
  styleUrl: './campi-segnalazione.component.css'
})
export class CampiSegnalazioneComponent implements OnInit {
  ...
  fieldRows: (SegnalazioneCampo & { typeLabel: string; scopeLabel: string; mandatoryLabel: string; multipleLabel: string })[] = [];
  fieldsLoading = false;
  fieldsError = '';

  fieldFormVisible = false;
  editingField: SegnalazioneCampo | null = null;
  fieldDraft: FieldDraft = this.emptyField();
  fieldSaveError = '';
  savingField = false;

  deleteFieldVisible = false;
  fieldToDelete: SegnalazioneCampo | null = null;
  deleteFieldError = '';
  deletingField = false;

  selectedField: SegnalazioneCampo | null = null;
  fieldDetailVisible = false;
  options: SegnalazioneCampoOpzione[] = [];
  optionsLoading = false;
  optionsError = '';
  optionFormVisible = false;
  editingOption: SegnalazioneCampoOpzione | null = null;
  optionDraft: OptionDraft = this.emptyOption();
  optionSaveError = '';
  savingOption = false;
  deleteOptionVisible = false;
  optionToDelete: SegnalazioneCampoOpzione | null = null;
  deleteOptionError = '';
  deletingOption = false;
  previewVisible = false;
  previewLoading = false;
  previewOptions = new Map<number, SegnalazioneCampoOpzione[]>();
  previewModel: SegnalazioneFormModel = { title: '', description: '', values: {}, attachments: {}, internal: false };
  ...
  loadFields(): void {
    ...
    this.fieldsError = '';
    this.api.segnalazioneCampi(projectId).pipe(finalize(() => this.fieldsLoading = false)).subscribe({
      next: fields => {
  ...
  saveField(): void {
    ...
    const request = editing
      ? this.api.aggiornaSegnalazioneCampo(editing.id, input)
      : this.api.creaSegnalazioneCampo({ ...input, projectId });
  ...
  confirmDeleteField(field: SegnalazioneCampo): void {
  ...
  deleteField(): void {
    ...
    this.deleteFieldError = '';
    this.api.eliminaSegnalazioneCampo(field.id).pipe(finalize(() => this.deletingField = false)).subscribe({
      next: () => {
      ...
      error: () => this.deleteFieldError = 'Impossibile eliminare il campo. Potrebbe essere usato da segnalazioni esistenti.'
    });
  }

  openField(field: SegnalazioneCampo): void {
    this.selectedField = field;
    this.editingField = field;
    this.fieldDraft = this.fieldDraftFrom(field);
    this.fieldSaveError = '';
  ...
  loadOptions(): void {
    ...
    this.optionsError = '';
    this.api.segnalazioneCampoOpzioni(field.id).pipe(finalize(() => this.optionsLoading = false)).subscribe({
      next: options => {
  ...
  editOption(option: SegnalazioneCampoOpzione): void {
  ...
  saveOption(): void {
    ...
    const request = this.editingOption
      ? this.api.aggiornaSegnalazioneCampoOpzione(this.editingOption.id, input)
      : this.api.creaSegnalazioneCampoOpzione({ definitionId: field.id, value: input.value, label: input.label });
    request.pipe(finalize(() => this.savingOption = false)).subscribe({
      ...
      error: (error: unknown) => {
        this.optionSaveError = error instanceof HttpErrorResponse && error.status === 409
          ? 'Opzione già usata o non rinominabile perché presente in segnalazioni esistenti.'
          : 'Impossibile salvare l’opzione. Controlla i dati e riprova.';
      }
    });
  }

  confirmDeleteOption(option: SegnalazioneCampoOpzione): void {
  ...
  deleteOption(): void {
    ...
    this.deleteOptionError = '';
    this.api.eliminaSegnalazioneCampoOpzione(option.id).pipe(finalize(() => this.deletingOption = false)).subscribe({
      next: () => {
  ...
  openPreview(): void {
    ...
    const requests = Object.fromEntries(selectFields.map(field => [
      field.id,
      this.api.segnalazioneCampoOpzioni(field.id)
    ]));
  ...
  previewFields(): SegnalazioneCampo[] {
  ...
  private fieldDraftFrom(field: SegnalazioneCampo): FieldDraft {
    return {
      code: field.code,
      label: field.label,
  ...
  private sortOptions(options: SegnalazioneCampoOpzione[]): SegnalazioneCampoOpzione[] {
