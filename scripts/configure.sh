...
${DOMAIN} {
    ...
    # cert generated by configure.sh. The Caddy → nginx hop is therefore
    # always encrypted, never plaintext. We force HTTP/1.1 on the TLS
    # channel so WebSocket upgrades work reliably — nginx upstream does
    # not implement RFC 8441 Extended CONNECT, which HTTP/2 upstream
